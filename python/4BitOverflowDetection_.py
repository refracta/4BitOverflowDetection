...
# %% Funtion of statement for negative detection.
def NEG(X, Y, carryIn = 0):
    S = getSum(X, Y, carryIn)
    result = S[3]
    return result

# %% Function of statement for zero detection.
def ZERO(X, Y, carryIn = 0):
    S = getSum(X, Y, carryIn)
    result = neg(S[0]) & neg(S[1]) & neg(S[2]) & neg(S[3]) 
    return result
...
# %% Export results.
resultOut = DataFrame(evaluateStatementWithEnter(NEG))
resultOut.to_csv('NEG.csv')

resultOut = DataFrame(evaluateStatementWithEnter(ZERO))
resultOut.to_csv('ZERO.csv')

resultOut = DataFrame(evaluateStatementWithEnter(OFD))
resultOut.to_csv('OFD.csv')
